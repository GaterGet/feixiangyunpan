package com.fx.pan.utils.office;
...
public class ExcelUtils {
    public static CellStyle createCellStyle(XSSFSheet sheet, XSSFWorkbook wb, JSONObject jsonObjectValue) {
        ...
        if (jsonObjectValue.get("un") != null) {
            font.setUnderline("1".equals(jsonObjectValue.get("un").toString()) ? FontUnderline.SINGLE :
                    FontUnderline.NONE);
        }
        //字体大小
    ...
    public static void exportLuckySheetXlsx(String excelData, HttpServletRequest request,
        ...
        for (int i = 0; i < jsonObjects.size(); i++) {
            ...
            //默认高
            short defaultRowHeight = jsonObject.getShort("defaultRowHeight") == null ? 20 : jsonObject.getShort(
                    "defaultRowHeight");
            //默认宽
            short defaultColWidth = jsonObject.getShort("defaultColWidth") == null ? 74 : jsonObject.getShort(
                    "defaultColWidth");
        ...
        try {
            ...
            if (request != null && request.getHeader("USER-AGENT") != null && StringUtils.contains(request.getHeader(
                    "USER-AGENT"), "Firefox")) {
            ...
            for (Map.Entry<String, Object> rowEntry : rowMap.entrySet()) {
                ...
                if (columnlenObject != null) {
                    ...
                    for (Map.Entry<String, Object> cloEntry : cloMap.entrySet()) {
                        ...
                        sheet.setColumnWidth(Integer.parseInt(cloEntry.getKey()), wid.multiply(excleWid).setScale(0,
                                BigDecimal.ROUND_HALF_UP).intValue());//列宽px值
        ...
        for (int index = 0; index < jsonObjectList.size(); index++) {
            ...
            if (jsonObjectValue.get("ps") != null) {
                ...
                XSSFClientAnchor anchor = new XSSFClientAnchor(colrowMapPS.get("dx1"), colrowMapPS.get("dy1"),
                        colrowMapPS.get("dx2"), colrowMapPS.get("dy2"), colrowMapPS.get("col1"), colrowMapPS.get(
        ...
        if (dataVerification != null) {
            ...
            for (Map.Entry<String, Object> dataEntry : dataVe.entrySet()) {
                ...
                CellRangeAddressList dstAddrList = new CellRangeAddressList(Integer.parseInt(colRow[0]),
                        Integer.parseInt(colRow[0]), Integer.parseInt(colRow[1]), Integer.parseInt(colRow[1]));//
                // 规则一单元格范围
                ...
                if ("dropdown".equals(dataVeValue.getString("type"))) {
                    if (dataVeValue.getString("value1").contains(",")) {
                        ...
                        dstDataValidation =
                                helper.createValidation(helper.createFormulaListConstraint(dataVeValue.getString(
                                        "value1")), dstAddrList);
                    }
                }
                ...
                if ("number_integer".equals(dataVeValue.getString("type"))
                    ...
                    DataValidationConstraint dvc =
                            helper.createNumericConstraint(opTypeMap.get(dataVeValue.getString("type")),
                                    opTypeMap.get(dataVeValue.getString("type2")), dataVeValue.getString("value1"),
                                    dataVeValue.getString("value2"));
                    dstDataValidation = helper.createValidation(dvc, dstAddrList);
                }
                if ("date".equals(dataVeValue.getString("type"))) {
                    //日期
                    DataValidationConstraint dvc =
                            new XSSFDataValidationConstraint(opTypeMap.get(dataVeValue.getString("type")),
                                    opTypeMap.get(dataVeValue.getString("type2")), dataVeValue.getString("value1"),
                                    dataVeValue.getString("value2"));
                    dstDataValidation = helper.createValidation(dvc, dstAddrList);
                }
