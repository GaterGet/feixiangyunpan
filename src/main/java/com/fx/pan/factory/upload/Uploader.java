package com.fx.pan.factory.upload;
...
public abstract class Uploader {
    ...
    private void rectifier(CosMultipartFile cosMultipartFile, UploadFile uploadFile) {
        ...
        try {
            ...
            if (redisCache.getCacheObject(current_upload_chunk_number) == null) {
                redisCache.set(current_upload_chunk_number, 1, 1000 * 60 * 60);
            }
            int currentUploadChunkNumber = Integer.parseInt(redisCache.getCacheObject(current_upload_chunk_number));
            ...
            if (uploadFile.getChunkNumber() != currentUploadChunkNumber) {
                ...
                while (redisLock.tryLock(key, 300, TimeUnit.SECONDS)) {
                    ...
                    currentUploadChunkNumber = Integer.parseInt(redisCache.getCacheObject(current_upload_chunk_number));
            ...
            redisCache.set("QiwenUploader:Identifier:" + uploadFile.getIdentifier() + ":current_upload_chunk_number", uploadFile.getChunkNumber(), 1000 * 60 * 60);
